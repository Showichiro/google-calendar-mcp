...
export const updateRecurringEventSchema = z.object({
  ...
  scope: z.enum(['thisEventOnly', 'all', 'thisAndFollowing']).describe('編集スコープ'),
  instanceDate: z.string().optional().describe('特定インスタンスの日付（thisEventOnly/thisAndFollowing時に必要、YYYY-MM-DD形式）'),
  instanceDateTime: z.string().optional().describe('特定インスタンスの日時（thisEventOnly/thisAndFollowing時に必要、ISO 8601形式）'),
...
export const deleteRecurringInstanceSchema = z.object({
  calendarId: z.string().default('primary').describe('カレンダーID'),
  eventId: z.string().describe('定期イベントの親イベントID'),
  instanceDate: z.string().optional().describe('削除するインスタンスの日付（YYYY-MM-DD形式）'),
  instanceDateTime: z.string().optional().describe('削除するインスタンスの日時（ISO 8601形式）'),
  sendUpdates: z.enum(['all', 'externalOnly', 'none']).optional().default('all').describe('参加者への通知設定'),
});

// ヘルパー関数：インスタンスIDの生成
function generateInstanceId(eventId: string, instanceDate?: string, instanceDateTime?: string): string {
  if (instanceDateTime) {
...
function addUntilToRRule(recurrence: string[], untilDate: string): string[] {
  return recurrence.map(rule => {
    if (rule.startsWith('RRULE:')) {
      ...
      const untilString = until.toISOString().replace(/[-:]/g, '').split('T')[0] + 'T235959Z';
...
// ツール実装
export async function updateRecurringEvent(params: z.infer<typeof updateRecurringEventSchema>) {
  const validatedParams = updateRecurringEventSchema.parse(params);
  const calendar = await getCalendarClient();

  const { calendarId, eventId, scope, instanceDate, instanceDateTime, ...updates } = validatedParams;
  ...
  switch (scope) {
    case 'thisEventOnly': {
      // 特定のインスタンスのみを更新
      const instanceId = generateInstanceId(eventId, instanceDate, instanceDateTime);
    ...
    case 'thisAndFollowing': {
      ...
      const splitDate = instanceDate || (instanceDateTime ? instanceDateTime.split('T')[0] : undefined);
      if (!splitDate) {
        throw new Error('instanceDate または instanceDateTime を指定してください');
      }

      const updatedRecurrence = addUntilToRRule(originalEvent.data.recurrence, splitDate);
      ...
      const newEventBody: Record<string, unknown> = {
        ...
        description: updates.description !== undefined ? updates.description : originalEvent.data.description,
        location: updates.location !== undefined ? updates.location : originalEvent.data.location,
        start: updates.start || {
        ...
        colorId: updates.colorId !== undefined ? updates.colorId : originalEvent.data.colorId,
      ...
      return {
        ...
        message: 'これ以降の予定を更新しました（元のイベントを分割して新しい定期イベントを作成）',
      };
    }
...
export async function deleteRecurringInstance(params: z.infer<typeof deleteRecurringInstanceSchema>) {
  const validatedParams = deleteRecurringInstanceSchema.parse(params);
  const calendar = await getCalendarClient();

  const { calendarId, eventId, instanceDate, instanceDateTime, sendUpdates } = validatedParams;
  ...
  return {
    ...
    message: `定期イベントの特定インスタンス (${instanceDate || instanceDateTime}) を削除しました`,
  };
}
...
export const recurringTools = [
  {
    ...
    description: '定期イベント（繰り返しイベント）を更新します。「この予定のみ」「すべての予定」「これ以降の予定」の3つの編集スコープをサポートします',
    inputSchema: {
      ...
      properties: {
        ...
        scope: {
          ...
          description: '編集スコープ: thisEventOnly=この予定のみ, all=すべての予定, thisAndFollowing=これ以降の予定',
        },
        instanceDate: {
          type: 'string',
          description: '特定インスタンスの日付（終日イベントの場合、YYYY-MM-DD形式）。thisEventOnly/thisAndFollowing時に必要',
        },
        instanceDateTime: {
          type: 'string',
          description: '特定インスタンスの日時（ISO 8601形式）。thisEventOnly/thisAndFollowing時に必要',
        },
        summary: {
          type: 'string',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        instanceDate: {
          type: 'string',
          description: '削除するインスタンスの日付（終日イベントの場合、YYYY-MM-DD形式）',
        },
        instanceDateTime: {
          type: 'string',
