...
export const getFreeBusySchema = z.object({
  ...
  calendarIds: z.array(z.string()).optional().default(['primary']).describe('確認するカレンダーIDのリスト'),
...
export const utilTools = [
  {
    name: 'get_freebusy',
    description: '指定した期間の空き時間情報を取得します。複数のカレンダーの予定が埋まっている時間帯を確認できます',
    inputSchema: {
      type: 'object' as const,
      properties: {
        timeMin: {
          type: 'string',
          description: '確認開始日時（ISO 8601形式、例: 2024-01-15T00:00:00+09:00）',
        },
        timeMax: {
          type: 'string',
          description: '確認終了日時（ISO 8601形式、例: 2024-01-15T23:59:59+09:00）',
        },
        calendarIds: {
          type: 'array',
          items: { type: 'string' },
          description: '確認するカレンダーIDのリスト（デフォルト: ["primary"]）',
        },
        timeZone: {
          type: 'string',
