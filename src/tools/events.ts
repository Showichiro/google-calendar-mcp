...
export const listEventsSchema = z.object({
  ...
  timeMin: z.string().optional().describe('取得開始日時（ISO 8601形式、例: 2024-01-01T00:00:00Z）'),
  ...
  singleEvents: z.boolean().optional().default(true).describe('定期イベントを個別インスタンスに展開するか'),
  orderBy: z.enum(['startTime', 'updated']).optional().default('startTime').describe('ソート順'),
});
...
export const createEventSchema = z.object({
  ...
  start: z.object({
    dateTime: z.string().optional().describe('開始日時（ISO 8601形式、例: 2024-01-15T10:00:00+09:00）'),
    date: z.string().optional().describe('終日イベントの開始日（YYYY-MM-DD形式）'),
    timeZone: z.string().optional().describe('タイムゾーン（例: Asia/Tokyo）'),
  }).describe('開始日時'),
  end: z.object({
    dateTime: z.string().optional().describe('終了日時（ISO 8601形式）'),
    date: z.string().optional().describe('終日イベントの終了日（YYYY-MM-DD形式）'),
  ...
  recurrence: z.array(z.string()).optional().describe('繰り返しルール（RRULE形式、例: ["RRULE:FREQ=WEEKLY;COUNT=10"]）'),
  reminders: z.object({
    useDefault: z.boolean().optional().describe('デフォルトのリマインダーを使用するか'),
  ...
  visibility: z.enum(['default', 'public', 'private', 'confidential']).optional().describe('公開設定'),
  transparency: z.enum(['opaque', 'transparent']).optional().describe('予定あり(opaque)/予定なし(transparent)'),
});
...
export const updateEventSchema = z.object({
  ...
  colorId: z.string().optional().describe('イベントの色ID'),
  visibility: z.enum(['default', 'public', 'private', 'confidential']).optional().describe('公開設定'),
  transparency: z.enum(['opaque', 'transparent']).optional().describe('予定あり/予定なし'),
});

export const deleteEventSchema = z.object({
  calendarId: z.string().default('primary').describe('カレンダーID'),
  eventId: z.string().describe('削除するイベントのID'),
  sendUpdates: z.enum(['all', 'externalOnly', 'none']).optional().default('all').describe('参加者への通知設定'),
});
...
export async function updateEvent(params: z.infer<typeof updateEventSchema>) {
  ...
  if (validatedParams.summary !== undefined) updateBody.summary = validatedParams.summary;
  if (validatedParams.description !== undefined) updateBody.description = validatedParams.description;
  if (validatedParams.location !== undefined) updateBody.location = validatedParams.location;
  if (validatedParams.start !== undefined) updateBody.start = validatedParams.start;
  if (validatedParams.end !== undefined) updateBody.end = validatedParams.end;
  if (validatedParams.attendees !== undefined) updateBody.attendees = validatedParams.attendees;
  if (validatedParams.colorId !== undefined) updateBody.colorId = validatedParams.colorId;
  if (validatedParams.visibility !== undefined) updateBody.visibility = validatedParams.visibility;
  if (validatedParams.transparency !== undefined) updateBody.transparency = validatedParams.transparency;
...
export const eventTools = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        singleEvents: {
          type: 'boolean',
          description: '定期イベントを個別インスタンスに展開するか（デフォルト: true）',
        },
        orderBy: {
          type: 'string',
  ...
  {
    name: 'create_event',
    description: '新しいカレンダーイベントを作成します。定期イベントの作成も可能です',
    inputSchema: {
      type: 'object' as const,
      properties: {
        calendarId: {
        ...
        start: {
          type: 'object',
          properties: {
            dateTime: { type: 'string', description: '開始日時（ISO 8601形式、例: 2024-01-15T10:00:00+09:00）' },
            date: { type: 'string', description: '終日イベントの開始日（YYYY-MM-DD形式）' },
          ...
          type: 'object',
          properties: {
            dateTime: { type: 'string', description: '終了日時（ISO 8601形式）' },
            date: { type: 'string', description: '終日イベントの終了日（YYYY-MM-DD形式）' },
        ...
        recurrence: {
          type: 'array',
          items: { type: 'string' },
          description: '繰り返しルール（RRULE形式、例: ["RRULE:FREQ=WEEKLY;COUNT=10"]）',
        },
        reminders: {
          type: 'object',
          properties: {
            useDefault: { type: 'boolean', description: 'デフォルトのリマインダーを使用' },
            overrides: {
              type: 'array',
              items: {
                type: 'object',
